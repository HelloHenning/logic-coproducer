...
enum MIDIFixtureGenerator {
    static func writeGoldenA1Fixture(to midiURL: URL) throws -> URL {
        let ppq = 960
        let noteCount = 1_024
        ...
        // Non-note events are distributed across the same timeline so A1 can
        // verify Logic's filter/status handling as well as note rows.
        ...
        var channelPressureCount = 0
        var polyPressureCount = 0
        ...
        let counts = FixtureCounts(
            ...
            programChanges: 4,
            expectedEventListRows: noteCount + controllerCount + pitchBendCount + channelPressureCount + polyPressureCount + 4
        )
