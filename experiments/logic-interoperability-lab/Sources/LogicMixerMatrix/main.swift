...
private enum ControlKind: String, Codable, CaseIterable {
    case volume = "Volume"
    case pan = "Pan"
    case mute = "Mute"
    case solo = "Solo"
...
private struct StripTopology: Codable {
...
private struct TopologyResult: Codable {
    let schema: String
...
private func isMixerPath(_ path: String) -> Bool {
...
private func controlRefs(_ refs: [Ref], kind: ControlKind) -> [Ref] {
    refs.filter { ref in
        let c = ref.candidate
...
private func values(_ refs: [Ref], kind: ControlKind) -> [String: Double] {
...
private func resolve(_ refs: [Ref], path: String) -> Ref? { refs.first { $0.candidate.path == path } }

private func changedPaths(before: [String: Double], after: [String: Double], excluding target: String) -> [String] {
...
private func allEqual(_ a: [String: Double], _ b: [String: Double]) -> Bool {
...
if command == "topology" {
    ...
    let result = TopologyResult(schema: "logic-coproducer-mixer-topology/1.0", generatedAt: nowISO(), visitedNodes: firstScan.visited, strips: strips, selectedElements: Array(selected))
    do { try writeJSON(result, path: out) } catch { fputs("Could not write topology: \(error)\n", stderr); exit(6) }
...
if initialTargets.isEmpty {
    let result = MatrixResult(schema: "logic-coproducer-mixer-matrix/1.0", generatedAt: nowISO(), control: kind.rawValue, candidateCount: 0, passCount: 0, failCount: 0, skipCount: 1, restoreFailureCount: 0, cases: [])
...
for (index, original) in initialTargets.enumerated() {
    ...
    let changedCorrectly: Bool
    if let observed, let requested {
        changedCorrectly = abs(observed - requested) <= 1e-9
    } else {
        changedCorrectly = false
    }

    if !changedCorrectly {
        // Attempt restoration even if the reported write did not read back as requested.
    ...
    if kind.isNumeric { restoreError = AX.setNumber(changedTarget.element, before) }
    else { restoreError = AX.press(changedTarget.element) }
    usleep(300_000)
...
let result = MatrixResult(schema: "logic-coproducer-mixer-matrix/1.0", generatedAt: nowISO(), control: kind.rawValue, candidateCount: initialTargets.count, passCount: passCount, failCount: failCount, skipCount: skipCount, restoreFailureCount: restoreFailureCount, cases: cases)
do { try writeJSON(result, path: out) } catch { fputs("Could not write matrix result: \(error)\n", stderr); exit(6) }
